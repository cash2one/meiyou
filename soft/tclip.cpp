...
int detectCharacter( Mat img ){
    ...
    for (vector<KeyPoint>::iterator i = keypoints.begin(); i != keypoints.end(); i++)
    {
        ...
        {
            section_index = (int)ceil(i->pt.y / section_length);
    ...
    if (Y > 0)
    {
        nRet = (Y - con_num / 2 + 1) * section_length;
    }
    else
    {
        nRet = con_num / 2 * section_length;
    }
...
int main(int argc, char** argv)
{
    ...
    if (ratio_width > ratio_height) {
        if (result == -1) {
            ...
            int nResultAdj = result - dest_height/2;
            nResultAdj = nResultAdj < 0 ? 0 : nResultAdj;
            if (dest_image.size().height - nResultAdj >= dest_height) {
                clip_top = -nResultAdj;
                clip_bottom = -(dest_image.size().height - nResultAdj - dest_height);
            } else {
